...
export default function DoorElden() {
    ...
    return (
        <div className="relative w-full h-screen overflow-hidden bg-gradient-to-b from-gray-900 via-gray-800 to-black">
            ...
            <div className="relative flex items-center justify-center h-full">
                ...
                >
                    ...
                    <div className="relative w-64 h-96 md:w-80 md:h-[32rem]">
                        {/* Glow effect */}
                        <div className={`absolute inset-0 bg-amber-500/30 blur-3xl transition-opacity duration-700 ${isOpening ? 'opacity-100' : 'opacity-0 group-hover:opacity-50'
                            }`}></div>

                        {/* Left door */}
                        <div className={`absolute left-0 top-0 w-1/2 h-full bg-gradient-to-br from-amber-900/80 via-amber-800/60 to-gray-900/80 
              border-2 border-amber-700/50 transition-all duration-1000 origin-left ${isOpening ? '-translate-x-full rotate-y-90 opacity-0' : ''
                            }`}>
                        ...
                        {/* Right door */}
                        <div className={`absolute right-0 top-0 w-1/2 h-full bg-gradient-to-bl from-amber-900/80 via-amber-800/60 to-gray-900/80 
              border-2 border-amber-700/50 transition-all duration-1000 origin-right ${isOpening ? 'translate-x-full rotate-y-90 opacity-0' : ''
                            }`}>
                            <div className="absolute inset-2 border border-amber-600/30"></div>
                    ...
                    <div className={`hover:cursor-pointer absolute inset-0 bg-gradient-to-t from-amber-400/0 via-amber-300/20 to-amber-200/40 transition-opacity duration-1000 ${isOpening ? 'opacity-100' : 'opacity-0'
                        }`}></div>

                    {/* Hover prompt */}
                    <div className={`absolute -bottom-16 left-1/2 -translate-x-1/2 text-amber-400 text-sm md:text-base font-semibold tracking-wider transition-opacity ${isOpening ? 'opacity-0' : 'opacity-0 group-hover:opacity-100'
                        }`}>
                        Open
                    </div>
                </button>
            ...
            <div className={`absolute inset-0 bg-white transition-opacity duration-500 pointer-events-none ${showFlash ? 'opacity-100' : 'opacity-0'
                }`}></div>

            {/* Title */}
            <div className="absolute top-8 left-1/2 -translate-x-1/2 text-center">
                <h1 className="text-2xl md:text-4xl font-bold text-amber-400 tracking-widest mb-2" style={{ fontFamily: 'serif' }}>
                    THE DOOR
                </h1>
